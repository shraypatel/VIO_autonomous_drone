...
Subscribes to:
  ...
  - input_rc (px4_msgs/InputRc): only if use_rc_pilot_mux:=true; RC sticks from FC / QGC
...
RC pilot mux (optional, parameter use_rc_pilot_mux:=true): when the pilot deflects sticks,
velocity commands follow the sticks. After neutral + rc_idle_timeout_sec, navigation resumes.
Default is ROS path control only (use_rc_pilot_mux:=false).
...
class PathExecutorNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("use_rc_pilot_mux", False)
        self.declare_parameter("input_rc_topic", "/fmu/out/input_rc")
        self.declare_parameter("rc_idle_timeout_sec", 0.75)
        self.declare_parameter("rc_pwm_center", 1500)
        self.declare_parameter("rc_pwm_half_span", 500.0)
        self.declare_parameter("rc_deadband_norm", 0.08)
        self.declare_parameter("rc_throttle_deadband_norm", 0.12)
        self.declare_parameter("rc_channel_roll", 0)
        self.declare_parameter("rc_channel_pitch", 1)
        self.declare_parameter("rc_channel_throttle", 2)
        self.declare_parameter("rc_channel_yaw", 3)
        self.declare_parameter("rc_sign_roll", -1.0)
        self.declare_parameter("rc_sign_pitch", -1.0)
        self.declare_parameter("rc_sign_yaw", -1.0)
        self.declare_parameter("rc_sign_throttle", 1.0)
        self.declare_parameter("rc_max_linear", 0.6)
        self.declare_parameter("rc_max_angular", 0.8)
        self.declare_parameter("rc_max_vertical", 0.5)
        ...
        self.use_rc_pilot_mux = self.get_parameter("use_rc_pilot_mux").value
        input_rc_topic = self.get_parameter("input_rc_topic").value
        self.rc_idle_timeout_sec = self.get_parameter("rc_idle_timeout_sec").value
        self.rc_pwm_center = int(self.get_parameter("rc_pwm_center").value)
        self.rc_pwm_half_span = float(self.get_parameter("rc_pwm_half_span").value)
        self.rc_deadband_norm = float(self.get_parameter("rc_deadband_norm").value)
        self.rc_throttle_deadband_norm = float(
            self.get_parameter("rc_throttle_deadband_norm").value
        )
        self.rc_channel_roll = int(self.get_parameter("rc_channel_roll").value)
        self.rc_channel_pitch = int(self.get_parameter("rc_channel_pitch").value)
        self.rc_channel_throttle = int(self.get_parameter("rc_channel_throttle").value)
        self.rc_channel_yaw = int(self.get_parameter("rc_channel_yaw").value)
        self.rc_sign_roll = float(self.get_parameter("rc_sign_roll").value)
        self.rc_sign_pitch = float(self.get_parameter("rc_sign_pitch").value)
        self.rc_sign_yaw = float(self.get_parameter("rc_sign_yaw").value)
        self.rc_sign_throttle = float(self.get_parameter("rc_sign_throttle").value)
        self.rc_max_linear = float(self.get_parameter("rc_max_linear").value)
        self.rc_max_angular = float(self.get_parameter("rc_max_angular").value)
        self.rc_max_vertical = float(self.get_parameter("rc_max_vertical").value)
        ...
        self.input_rc_sub = None
        if self.use_rc_pilot_mux:
            self.input_rc_sub = self.create_subscription(
                InputRc, input_rc_topic, self.input_rc_callback, 10
            )
        ...
        self._last_rc_stick_activity_time = None
        self._input_rc_received = False
        self._last_input_rc_msg = None
        ...
        self.get_logger().info(
            "Path executor: path={}, cmd_vel={}, enable={}, use_rc_pilot_mux={}, input_rc={}".format(
                path_topic,
                cmd_vel_topic,
                enable_topic,
                self.use_rc_pilot_mux,
                input_rc_topic if self.use_rc_pilot_mux else "(disabled)",
            )
        )
    ...
    def _pwm_to_norm(self, pwm: int) -> float:
        x = (float(pwm) - float(self.rc_pwm_center)) / max(self.rc_pwm_half_span, 1e-3)
        return max(-1.0, min(1.0, x))

    def _sticks_deflected(self, msg: InputRc) -> bool:
        if msg.rc_lost or msg.rc_failsafe:
            return False
        n = int(msg.channel_count)
        need = (
            max(
                self.rc_channel_roll,
                self.rc_channel_pitch,
                self.rc_channel_throttle,
                self.rc_channel_yaw,
            )
            + 1
        )
        ...
        def over_deadband(norm: float, db: float) -> bool:
            return abs(norm) > db

        r = self._pwm_to_norm(int(msg.values[self.rc_channel_roll]))
        p = self._pwm_to_norm(int(msg.values[self.rc_channel_pitch]))
        t = self._pwm_to_norm(int(msg.values[self.rc_channel_throttle]))
        y = self._pwm_to_norm(int(msg.values[self.rc_channel_yaw]))

        if over_deadband(r, self.rc_deadband_norm):
            return True
        if over_deadband(p, self.rc_deadband_norm):
            return True
        if over_deadband(y, self.rc_deadband_norm):
            return True
        if over_deadband(t, self.rc_throttle_deadband_norm):
            return True
        return False

    def input_rc_callback(self, msg: InputRc):
        self._input_rc_received = True
        self._last_input_rc_msg = msg
        if self._sticks_deflected(msg):
            self._last_rc_stick_activity_time = self.get_clock().now()

    def _rc_mux_active(self) -> bool:
        """True until rc_idle_timeout_sec after the last deflected-stick RC sample."""
        if not self.use_rc_pilot_mux:
            return False
        if not self._input_rc_received or self._last_rc_stick_activity_time is None:
            return False
        dt = (
            self.get_clock().now() - self._last_rc_stick_activity_time
        ).nanoseconds * 1e-9
        return dt < self.rc_idle_timeout_sec

    def _publish_rc_twist(self):
        """Map current RC channels to body-frame velocity (offboard)."""
        twist = Twist()
        enable = Bool()
        enable.data = True
        msg = self._last_input_rc_msg
        if msg is None or msg.rc_lost or msg.rc_failsafe:
            self.publish_stop("RC lost/failsafe while mux active: stop")
            return
        ...
        if n < need:
            self.publish_stop("RC channel_count too low: stop")
            return

        roll_n = self._pwm_to_norm(int(msg.values[self.rc_channel_roll]))
        pitch_n = self._pwm_to_norm(int(msg.values[self.rc_channel_pitch]))
        throttle_n = self._pwm_to_norm(int(msg.values[self.rc_channel_throttle]))
        yaw_n = self._pwm_to_norm(int(msg.values[self.rc_channel_yaw]))

        def apply_deadband(v: float, db: float) -> float:
            return 0.0 if abs(v) <= db else v

        roll_n = apply_deadband(roll_n, self.rc_deadband_norm)
        pitch_n = apply_deadband(pitch_n, self.rc_deadband_norm)
        throttle_n = apply_deadband(throttle_n, self.rc_throttle_deadband_norm)
        yaw_n = apply_deadband(yaw_n, self.rc_deadband_norm)

        # Body frame: x forward, y left, z up — scale by RC signs (airframe-specific).
        ...
        twist.linear.y = float(
            self.rc_sign_roll * roll_n * self.rc_max_linear * self.cmd_vel_body_y_gain
        )
        twist.linear.z = float(self.rc_sign_throttle * throttle_n * self.rc_max_vertical)
        twist.angular.z = float(self.rc_sign_yaw * yaw_n * self.rc_max_angular)
    ...
    def control_callback(self):
        ...
        if self._rc_mux_active():
            self._publish_rc_twist()
            return
